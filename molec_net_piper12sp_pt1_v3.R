###### Part 1 of R function to calculate Chemical Structural Similarity (CSS), adapted from Sedio et al.Applications in Plant Sciences 2018 
...
molecNetsTraits = function(code, date, clone = TRUE, outfile = paste("MolecNetsChemTraits", date, ".RData",sep="")){
	wkdir = getwd()
	network = read.table("C:/Users/T530/Google Drive/Whitehead lab/piper_momics_17-18/Cytoscape data/combined_cscs/ProteoSAFe-METABOLOMICS-SNETS-b7ed685e-view_network/networkedges_selfloop/3e498e23668742138671bd51c08d2971.pairsinfo", header = TRUE)
	network$CLUSTERID1 = as.factor(network$CLUSTERID1)
	network$CLUSTERID2 = as.factor(network$CLUSTERID2)
	netshort = read.table("C:/Users/T530/Google Drive/Whitehead lab/piper_momics_17-18/Cytoscape data/combined_cscs/ProteoSAFe-METABOLOMICS-SNETS-b7ed685e-view_network/networkedges/4a515a24d4db41d29edadb6dd63bdc1c.pairsinfo", header = TRUE)
	...
	sampmass = read.csv("sampmass.csv", header = T)
	sampmass$sampmass = as.numeric(as.character(sampmass$sampmass))
	atts = read.table("C:/Users/T530/Google Drive/Whitehead lab/piper_momics_17-18/Cytoscape data/combined_cscs/ProteoSAFe-METABOLOMICS-SNETS-b7ed685e-view_network/clusterinfosummarygroup_attributes/24ae481144eb43f5a628db324ba1ce82..out", header = TRUE, comment.char = "")
	atts$cluster.index = as.factor(atts$cluster.index)
	blanks = sort(union(grep("blank", atts$AllFiles), grep("blank", atts$AllFiles)))
	defs = atts[-blanks,]

...
#singular fit issues, go away without random effect and no matter what
#there is a clear interaction between species and tissue
...
lmm.byspecies <- data.frame(species=character(), X=numeric(), P=numeric(), leaf=character(), 
                             seed=character(), unripe=character(), ripe=character())
SD$tissue <- factor(SD$tissue, levels=c("leaf","seed", "unripe", "ripe"))
SD$PlantID <- as.factor(SD$PlantID)
for(i in 1: length(levels(SD$species))){
  ...
  newrow <- data.frame(species=as.character(species), X=as.numeric(X), P=as.numeric(P), 
                       leaf=as.character(l[1]), seed=as.character(l[2]), unripe=as.character(l[3]),
                       ripe=as.character(l[4]))
  lmm.byspecies <- rbind(lmm.byspecies, newrow)
}
